...
function analyzeSeaColorDistribution(): void {
  ...
  const overallAverage = allCounts.reduce((sum, count) => sum + count, 0) / allCounts.length;
  const overallVariance = allCounts.reduce((sum, count) => sum + Math.pow(count - overallAverage, 2), 0) / allCounts.length;
  const overallStdDev = Math.sqrt(overallVariance);
  
  console.log("=== Overall Statistics ===");
  console.log(`Total sea tiles across all colors: ${allCounts.reduce((sum, count) => sum + count, 0)}`);
  ...
  // Check for imbalance
  const maxDifference = Math.max(...colors.map(color => colorStats[color].max)) - Math.min(...colors.map(color => colorStats[color].min));
