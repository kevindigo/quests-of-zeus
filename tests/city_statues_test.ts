...
Deno.test("City Statue Functionality - Basic Operations", () => {
  ...
  cities.forEach((city, _index) => {
    ...
    for (let i = 2; i >= 0; i--) {
      ...
      assertEquals(currentStatues, i, `Statue count should be ${i} after removal`);
    }
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      assertEquals(currentStatues, i, `Statue count should be ${i} after addition`);
    }
...
Deno.test("City Statue Functionality - Complete Cities", () => {
  ...
  assertEquals(completeCities.length, allCities.length, "All cities should be complete initially");
  ...
  if (allCities.length > 0) {
    ...
    assertEquals(updatedCompleteCities.length, allCities.length - 1, "Should have one less complete city");
  }
});