...
Deno.test("RecolorFavorCalculation - basic recoloring intention", () => {
  ...
  assertEquals(player.favor, 5, "Player favor should not be spent when setting intention");
});
...
Deno.test("RecolorFavorCalculation - moves account for recoloring cost", () => {
  ...
  for (const move of movesToPinkTiles) {
    ...
    assert(canAfford, `Move to (${move.q}, ${move.r}) should be affordable with total cost ${totalCost}`);
  }
});
...
Deno.test("RecolorFavorCalculation - high recoloring cost limits moves", () => {
  ...
  const highRecolorSuccess = gameEngine.setRecolorIntention(player.id, "black", 2); // black → blue (2 favor recoloring cost)
  
  assert(highRecolorSuccess, "High recoloring intention should be set successfully");
  ...
  for (const move of movesWithHighRecolor) {
    ...
    const canAfford = totalCost <= player.favor;
    assert(canAfford, `Move to (${move.q}, ${move.r}) should not be available with total cost ${totalCost} exceeding favor ${player.favor}`);
  }
});
...
Deno.test("RecolorFavorCalculation - moves without recoloring unaffected", () => {
  ...
  const movesWithoutRecolor = gameEngine.getAvailableMovesForDie(player.id, "black", player.favor);
  ...
  assertEquals(movesWithoutRecolor.length, movesToBlackTiles.length, "All moves should be to black sea tiles without recoloring");
  
  for (const move of movesWithoutRecolor) {
    const canAfford = move.favorCost <= player.favor;
    assert(canAfford, `Move to (${move.q}, ${move.r}) should be affordable with cost ${move.favorCost}`);
  }
});