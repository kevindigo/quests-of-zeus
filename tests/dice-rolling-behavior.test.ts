  ...
  state.players.forEach((player, index) => {
    ...
    player.oracleDice.forEach((dieColor, dieIndex) => {
      assert(
        ...
        `Player ${index + 1} die ${dieIndex} should be a valid color, got ${dieColor}`
      );
    });
  });
...
Deno.test("DiceRolling - dice rolled for next player at end of turn", () => {
  ...
  assertEquals(initialState.currentPlayerIndex, 0, "Player 1 should be current player initially");
...
Deno.test("DiceRolling - round advances when all players have taken turns", () => {
  ...
  assertEquals(stateAfterPlayer1Turn.round, 1, "Round should still be 1 after first endTurn");
...
Deno.test("DiceRolling - dice are valid colors after rolling", () => {
  ...
  const state = engine.getGameState();
  const validColors: HexColor[] = ["red", "pink", "blue", "black", "green", "yellow"];
  ...
  state.players.forEach((player, playerIndex) => {
    player.oracleDice.forEach((dieColor, dieIndex) => {
      assert(
        ...
        `Player ${playerIndex + 1} die ${dieIndex} should be valid color, got ${dieColor}`
      );
    });
  });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    newState.players.forEach((player, playerIndex) => {
      player.oracleDice.forEach((dieColor, dieIndex) => {
        assert(
          validColors.includes(dieColor),
          `Player ${playerIndex + 1} die ${dieIndex} should be valid color after endTurn, got ${dieColor}`
        );
      });
    });
  }
