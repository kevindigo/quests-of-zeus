...
function testMonsterDistribution(): void {
  ...
  if (monsterHexes2p.length !== 9) {
    console.error(`❌ Expected 9 monster hexes but got ${monsterHexes2p.length}`);
    return;
  }
  ...
  for (const color of Object.keys(colorCounts2p)) {
    if (colorCounts2p[color] !== expectedPerColor2p) {
      console.error(`❌ Expected ${expectedPerColor2p} monsters for color ${color} but got ${colorCounts2p[color]}`);
  ...
  for (const hex of monsterHexes2p) {
    ...
    if (uniqueColors.size !== hex.monsterColors.length) {
      console.error(`❌ Hex (${hex.q},${hex.r}) has duplicate colors: ${hex.monsterColors.join(', ')}`);
  ...
  const markedHexes2p = monsterHexes2p.filter(hex => hex.monsterColors.length === 2);
  ...
  const nonMarkedHexes2p = monsterHexes2p.filter(hex => hex.monsterColors.length !== 2);
