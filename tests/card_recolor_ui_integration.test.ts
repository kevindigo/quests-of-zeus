...
Deno.test("Card recolor UI integration test", () => {
  ...
  const recolorSuccess = gameEngine.setRecolorIntentionForCard(player.id, "black", 1);
  ...
  assertEquals(Object.keys(nextPlayer.recoloredCards || {}).length, 0, "Recoloring intentions should be cleared at end of turn");
  ...
  const recolorSuccess2 = gameEngine.setRecolorIntentionForCard(currentPlayer.id, "blue", 3);
  assert(recolorSuccess2, "Card recoloring intention should be set successfully");
  
  // Verify the recoloring intention
  assert(currentPlayer.recoloredCards && currentPlayer.recoloredCards["blue"], "Recoloring intention should be stored");
  assertEquals(currentPlayer.recoloredCards["blue"].newColor, "red", "Blue should recolor to red with 3 favor");
  assertEquals(currentPlayer.recoloredCards["blue"].favorCost, 3, "Recoloring cost should be 3 favor");
  
  // Test that player can still use other cards without recoloring
  const clearRecolorSuccess = gameEngine.clearRecolorIntentionForCard(currentPlayer.id, "pink");
  assert(clearRecolorSuccess, "Clearing recoloring intention should succeed");
  
  // Verify the recoloring was cleared for pink card
  assertEquals(Object.keys(currentPlayer.recoloredCards || {}).length, 1, "Should only have one recoloring intention");
  assert(currentPlayer.recoloredCards && currentPlayer.recoloredCards["blue"], "Blue card should still have recoloring intention");
});