...
function testCityNeighborsSimple() {
  ...
  try {
    ...
    for (const cityCell of cityCells) {
      ...
      const seaNeighbors = neighbors.filter((neighbor) =>
      ...
      if (seaNeighbors.length > 0) {
        ...
        console.log(
          `  - City at (${cityCell.q}, ${cityCell.r}): ${seaNeighbors.length} sea neighbors`,
        );
      }
    }
    ...
    if (citiesWithSeaNeighbors > 0) {
      ...
      console.log(
        "This indicates that the setRandomNeighborsToSea feature is working.",
      );
    } else {
      console.log("\n❌ FAILURE: No cities have sea neighbors!");
      console.log(
        "This suggests the setRandomNeighborsToSea feature may not be working correctly.",
      );
    }
  } catch (error) {
