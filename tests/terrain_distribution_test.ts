...
function testTerrainDistribution() {
  ...
  try {
    ...
    const missingTerrains = expectedTerrains.filter((terrain) =>
      !terrainCounts[terrain] || terrainCounts[terrain] === 0
    );
    ...
    console.log(`\nConstraint checks:`);
    console.log(
      `  Shallows: ${shallowCount} (should be 0-10 after sea-to-shallows conversion)`,
    );
    console.log(
      `  Sea: ${seaCount} (should be significant after 100% conversion)`,
    );
