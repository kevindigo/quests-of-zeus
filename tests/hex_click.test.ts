...
Deno.test('Hex click - player does not have that card', () => {
  setup();
...
Deno.test('Hex click - player does not have that die', () => {
  setup();
...
Deno.test('Hex click - attempt unlisted but otherwise legal move', () => {
  setup();
...
Deno.test('Hex click - move to listed option failed', () => {
  setup();
  const firstDie = player.oracleDice[0];
  assert(firstDie);
