...
Deno.test("Debug movement system", () => {
  ...
  console.log("Reachable sea tiles (with 5 favor):", reachableSeaTilesWithFavor.length);
  ...
  if (movementSystem) {
    const reachableTiles = movementSystem.getReachableSeaTiles(player.shipPosition.q, player.shipPosition.r, 3);
    console.log("Movement system reachable tiles (range 3):", reachableTiles.length);
    
    const reachableTilesWithExtendedRange = movementSystem.getReachableSeaTiles(player.shipPosition.q, player.shipPosition.r, 8);
    console.log("Movement system reachable tiles (range 8):", reachableTilesWithExtendedRange.length);
  }
});