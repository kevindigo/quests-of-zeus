...
function debugOfferingCubes() {
  ...
  try {
    ...
    const validCubeHexes = gameState.cubeHexes.filter(ch => 
      ch.cubes && ch.cubes.length === 1 && ch.cubes[0].count === gameState.players.length
    );
    
    if (validCubeHexes.length === 6) {
      console.log("✅ All cube hexes have exactly one color with correct count");
    } else {
      console.log(`❌ ${validCubeHexes.length} of 6 cube hexes have valid cube configuration`);
    }
    ...
    const duplicateColors = Object.entries(colorCounts).filter(([_, count]) => count > 1);
    if (duplicateColors.length === 0) {
      console.log("✅ No color appears twice on any island");
    } else {
      console.log(`❌ Found duplicate colors: ${duplicateColors.map(([color]) => color).join(', ')}`);
    }

    // Check cube counts
    const expectedCount = gameState.players.length;
    const correctCounts = gameState.cubeHexes.filter(ch => ch.cubes[0].count === expectedCount);
    if (correctCounts.length === 6) {
      console.log(`✅ All cube hexes have ${expectedCount} cubes (one per player)`);
    } else {
      console.log(`❌ ${correctCounts.length} of 6 cube hexes have correct cube count`);
    }

    console.log("\nCube hex distribution:");
    gameState.cubeHexes.forEach(ch => {
      const cube = ch.cubes[0];
      console.log(`  Position (${ch.q}, ${ch.r}): ${cube.color} cubes: ${cube.count}`);
    });
