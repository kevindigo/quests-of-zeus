...
function debugMonsterDistribution(): void {
  ...
  for (const hex of monsterHexes) {
    ...
    console.log(`Hex (${hex.q}, ${hex.r}) ${marker}: ${hex.monsterColors.join(', ')}`);
  }
  ...
  const markedHexes = monsterHexes.filter(hex => hex.monsterColors.length === 2);
  console.log(`✅ Marked hexes: ${markedHexes.length}/3 ${markedHexes.length === 3 ? "✓" : "✗"}`);
