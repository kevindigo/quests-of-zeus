...
const tilesBySteps: {
  [steps: number]: Array<{ q: number; r: number; color: string }>;
} = {};
reachableTiles.forEach((tile) => {
  // For now, we don't track steps in the return value, but we can estimate
  // This is just for debugging
  const distance = (engine as unknown as {
...
reachableTiles.forEach((tile) => {
  ...
  if (!availableMoveKeys.has(key)) {
    console.log(
      `  (${tile.q}, ${tile.r}) color ${tile.color} - missing from available moves`,
    );
  }
});
...
availableMoves.forEach((move) => {
  ...
  if (!reachableTileKeys.has(key)) {
    console.log(
      `  (${move.q}, ${move.r}) with ${move.dieColor} die - missing from reachable tiles`,
    );
  }
});
