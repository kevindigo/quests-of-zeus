...
function testRecolorMovementValidationFix() {
  ...
  console.log(
    `  Moves available after recoloring: ${movesAfterRecolor.length}`,
  );
  ...
  if (movesAfterRecolor.length > 0) {
    ...
    if (validationResult.success) {
      console.log(
        `  ✓ Validation fix working: Move succeeded with recolored die`,
      );
    } else {
      console.log(
        `  ✗ Validation fix not working: Move failed despite being shown as available`,
      );
