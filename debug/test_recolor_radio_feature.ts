...
function testRecolorRadioFeature() {
  ...
  console.log(
    `  Favor after intention: ${player.favor} (should be same as initial)\n`,
  );
  ...
  if (player.recoloredDice && player.recoloredDice["black"]) {
    console.log(
      `  ✓ Recoloring intention stored: ${
        player.recoloredDice["black"].newColor
      } for ${player.recoloredDice["black"].favorCost} favor`,
    );
  } else {
  ...
  if (
    clearSuccess && (!player.recoloredDice || !player.recoloredDice["black"])
  ) {
  ...
  console.log(
    `  Black die intention: ${player.recoloredDice["black"]?.newColor} (${
      player.recoloredDice["black"]?.favorCost
    } favor)`,
  );
  console.log(
    `  Pink die intention: ${player.recoloredDice["pink"]?.newColor} (${
      player.recoloredDice["pink"]?.favorCost
    } favor)`,
  );
  console.log(
    `  Blue die intention: ${player.recoloredDice["blue"]?.newColor} (${
      player.recoloredDice["blue"]?.favorCost
    } favor)`,
  );
  ...
  if (
    player.oracleDice[0] === "black" && player.oracleDice[1] === "pink" &&
    player.oracleDice[2] === "blue"
  ) {
    console.log(`  ✓ Dice colors unchanged: ${player.oracleDice.join(", ")}`);
  } else {
    console.log(
      `  ✗ Dice colors changed unexpectedly: ${player.oracleDice.join(", ")}`,
    );
  }
  ...
  const insufficientFavorResult = gameEngine.setRecolorIntention(
    player.id,
    "black",
    highFavorCost,
  );
  ...
  console.log(
    "Players can set recoloring intentions with radio buttons and change their mind freely.",
  );
}
