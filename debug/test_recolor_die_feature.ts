...
function testRecolorDie() {
  ...
  for (const testCase of testCases) {
    ...
    const success = gameEngine.setRecolorIntention(
      player.id,
      testCase.dieColor,
      testCase.favorSpent,
    );
    ...
    if (success) {
      ...
      if (
        originalDieIndex !== -1 &&
        player.oracleDice[originalDieIndex] === testCase.dieColor
      ) {
      ...
      console.log(
        `  Possible reasons: not enough favor, invalid die color, or not in action phase`,
      );
    }
  }
