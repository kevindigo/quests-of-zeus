...
for (const dieColor of player.oracleDice) {
  ...
  const availableMoves = gameEngine.getAvailableMovesForDie(player.id, dieColor, player.favor);
  ...
  availableMoves.forEach(move => {
    movesByFavorCost[move.favorCost] = (movesByFavorCost[move.favorCost] || 0) + 1;
  });
...
for (let favor = 0; favor <= 5; favor++) {
  const moves = gameEngine.getAvailableMovesForDie(player.id, testDieColor, favor);
  const movesWithFavor = moves.filter(move => move.favorCost > 0);
  
  console.log(`With ${favor} favor: ${moves.length} total moves, ${movesWithFavor.length} require favor`);
}
