...
console.log(`Player position: (${player1.shipPosition.q}, ${player1.shipPosition.r})`);
...
const invalidMoveResult = gameEngine.moveShip(player1.id, 100, 100, player1.oracleDice[0]);
console.log(`Invalid move result:`, invalidMoveResult);
console.log(`Current player after invalid move: ${gameEngine.getCurrentPlayer().name}`);
...
if (wrongColorHex) {
  console.log(`Attempting move to (${wrongColorHex.q}, ${wrongColorHex.r}) with wrong die color`);
  const wrongColorMoveResult = gameEngine.moveShip(player1.id, wrongColorHex.q, wrongColorHex.r, player1.oracleDice[0]);
  console.log(`Wrong color move result:`, wrongColorMoveResult);
  console.log(`Current player after wrong color move: ${gameEngine.getCurrentPlayer().name}`);
} else {
...
if (validMoves.length > 0) {
  ...
  console.log(`Attempting valid move to (${validMove.q}, ${validMove.r}) with die ${validMove.dieColor}`);
  const validMoveResult = gameEngine.moveShip(player1.id, validMove.q, validMove.r, validMove.dieColor);
  console.log(`Valid move result:`, validMoveResult);
  console.log(`Current player after valid move: ${gameEngine.getCurrentPlayer().name}`);
} else {
